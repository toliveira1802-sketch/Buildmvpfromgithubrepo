      ...
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold text-white flex items-center gap-2"><BarChart2 className="h-8 w-8 text-purple-400"/>Produtividade</h1>
            <p className="text-zinc-400 mt-1">Performance por mec�nico</p></div>
        ...
        {ranking.length > 0 && (
          <Card className="bg-zinc-900 border-zinc-800"><CardHeader><CardTitle className="text-white">OS por Mec�nico</CardTitle></CardHeader>
            <CardContent><ResponsiveContainer width="100%" height={220}>
              <BarChart data={ranking}><CartesianGrid strokeDasharray="3 3" stroke="#27272a"/>
                ...
                <Bar dataKey="concluidas" fill="#22c55e" name="Conclu�das" radius={[4,4,0,0]}/>
        ...
        <div className="space-y-3">
          ...
          : mecanicos.length === 0 ? <p className="text-zinc-500 text-sm text-center py-8">Nenhum mec�nico cadastrado em 12_MECANICOS</p>
          : mecanicos.map((m,i) => (
            <div key={m.id} className="flex items-center gap-4 p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
              ...
              <div className="flex gap-6 text-center">
                <div><p className="text-green-400 font-bold">{m.concluidas}</p><p className="text-zinc-500 text-xs">conclu�das</p></div>
